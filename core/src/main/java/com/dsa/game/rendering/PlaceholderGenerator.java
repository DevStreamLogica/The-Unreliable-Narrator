...
public class PlaceholderGenerator {
    ...
    static {
        ROOM_COLORS.put(Room.RoomID.ENTRANCE, new Color(0.35f, 0.3f, 0.25f, 1));
        ROOM_COLORS.put(Room.RoomID.STUDY, new Color(0.25f, 0.2f, 0.18f, 1));
        ROOM_COLORS.put(Room.RoomID.PARLOR, new Color(0.28f, 0.22f, 0.28f, 1));
        ROOM_COLORS.put(Room.RoomID.KITCHEN, new Color(0.22f, 0.22f, 0.25f, 1));
        ROOM_COLORS.put(Room.RoomID.GUEST_ROOMS, new Color(0.22f, 0.24f, 0.28f, 1));
        ROOM_COLORS.put(Room.RoomID.JAMES_ROOM, new Color(0.20f, 0.22f, 0.25f, 1));
        ROOM_COLORS.put(Room.RoomID.MARGARET_ROOM, new Color(0.24f, 0.26f, 0.30f, 1));
        ROOM_COLORS.put(Room.RoomID.GROUNDSKEEPER_SHED, new Color(0.18f, 0.22f, 0.18f, 1));
        ROOM_COLORS.put(Room.RoomID.SERVANTS_QUARTERS, new Color(0.22f, 0.2f, 0.18f, 1));
        ROOM_COLORS.put(Room.RoomID.CELLAR, new Color(0.12f, 0.1f, 0.08f, 1));
    }
    ...
    private static void addRoomDetails(Pixmap pixmap, Room.RoomID roomId, int width, int height, Color baseColor) {
        ...
        switch (roomId) {
            ...
            case JAMES_ROOM:
                drawBed(pixmap, width / 4, height - 150, 200, 120, baseColor);
                drawDresser(pixmap, width * 3 / 4, height - 180, 150, 100, baseColor);
                drawWindow(pixmap, width / 2, floorStart + 50, 120, 100, baseColor);
                break;

            case MARGARET_ROOM:
                drawBed(pixmap, width * 3 / 4, height - 150, 200, 120, baseColor);
                drawDresser(pixmap, width / 4, height - 180, 150, 100, baseColor);
                drawWindow(pixmap, width / 2, floorStart + 50, 120, 100, baseColor);
                break;
    ...
    private static void addLightingEffects(Pixmap pixmap, Room.RoomID roomId, int width, int height) {
        ...
        if (roomId == Room.RoomID.STUDY || roomId == Room.RoomID.GUEST_ROOMS || roomId == Room.RoomID.JAMES_ROOM || roomId == Room.RoomID.MARGARET_ROOM) {
            int lightX = width / 2;
